...
func (s Service) Apply() (core.ApplyResult, error) {
	...
	precheck, err := s.Doctor()
	if err != nil {
		return core.ApplyResult{}, err
	}
	if hasDoctorFailure(precheck.Findings) {
		_ = s.recordApplyState("fail", core.BackupRecord{})
		_ = s.logLifecycle("apply", "apply.precheck", "fail", string(core.ErrorCodeApplyPrecheck), "")
		return core.ApplyResult{}, &core.AppError{
			Code:       core.ErrorCodeApplyPrecheck,
			MessageKey: "error.apply.precheck",
			Cause:      fmt.Errorf("doctor pre-check reported failing findings"),
		}
	}
	...
	if err := schema.ValidateProfile(mustYAML(profile)); err != nil {
		_ = s.recordApplyState("fail", core.BackupRecord{})
		_ = s.logLifecycle("apply", "apply.profile_validation", "fail", string(core.ErrorCodeSchemaValidation), profile.Name)
		return core.ApplyResult{}, err
	}
	if err := validateProfileTarget(profile); err != nil {
		_ = s.recordApplyState("fail", core.BackupRecord{})
		_ = s.logLifecycle("apply", "apply.profile_constraints", "fail", string(core.ErrorCodeSchemaValidation), profile.Name)
		return core.ApplyResult{}, err
	}
	...
	if err != nil {
		_ = s.recordApplyState("fail", backupRecord)
		_ = s.logLifecycle("apply", "apply.verify", "fail", string(core.ErrorCodeApplyVerify), profile.Name)
		return core.ApplyResult{}, err
	}

	applyStatus := applyStateResult(verifyResultValue.Findings)
	if err := s.recordApplyState(applyStatus, backupRecord); err != nil {
		return core.ApplyResult{}, err
	}
	if applyStatus == "fail" {
		_ = s.logLifecycle("apply", "apply.verify", "fail", string(core.ErrorCodeApplyVerify), profile.Name)
		return core.ApplyResult{}, &core.AppError{
			...
			Cause:      fmt.Errorf("post-apply verification reported failing findings"),
		}
	}
	_ = s.logLifecycle("apply", "apply.completed", applyStatus, "", profile.Name)
...
func (s Service) Verify() (core.VerifyResult, error) {
	...
	if detectResult.OpenClawInstalled {
		findings = append(findings, core.VerifyFinding{
			Code:       "verify.openclaw.available",
			Severity:   core.SeverityPass,
			MessageKey: "verify.finding.openclaw.available",
		})
	} else {
		findings = append(findings, core.VerifyFinding{
			Code:       "verify.openclaw.available",
			Severity:   core.SeverityWarn,
			MessageKey: "verify.finding.openclaw.missing",
		})
	}

	if err := schema.ValidateProfile(mustYAML(profile)); err == nil {
		findings = append(findings, core.VerifyFinding{
			Code:       "verify.profile.schema",
			Severity:   core.SeverityPass,
			MessageKey: "verify.finding.profile.schema.valid",
		})
	} else {
		findings = append(findings, core.VerifyFinding{
			Code:       "verify.profile.schema",
			Severity:   core.SeverityFail,
			MessageKey: "verify.finding.profile.schema.invalid",
		})
	}

	if err := validateProfileTarget(profile); err == nil {
		findings = append(findings, core.VerifyFinding{
			Code:       "verify.profile.constraints",
			Severity:   core.SeverityPass,
			MessageKey: "verify.finding.profile.constraints.valid",
		})
	} else {
		findings = append(findings, core.VerifyFinding{
			Code:       "verify.profile.constraints",
			Severity:   core.SeverityFail,
			MessageKey: "verify.finding.profile.constraints.invalid",
		})
	}

	if data, err := os.ReadFile(plan.GeneratedConfig); err == nil {
		findings = append(findings, core.VerifyFinding{
		...
		if err := schema.ValidateProfile(data); err == nil {
			findings = append(findings, core.VerifyFinding{
				Code:       "verify.generated_config.schema",
				Severity:   core.SeverityPass,
				MessageKey: "verify.finding.generated_config.schema.valid",
			})
		} else {
			findings = append(findings, core.VerifyFinding{
				Code:       "verify.generated_config.schema",
				Severity:   core.SeverityFail,
				MessageKey: "verify.finding.generated_config.schema.invalid",
			})
		}
	} else {
		findings = append(findings, core.VerifyFinding{
	...
	if len(plan.ContentDiffs) == 0 {
		findings = append(findings, core.VerifyFinding{
			Code:       "verify.generated_config.sync",
			Severity:   core.SeverityPass,
			MessageKey: "verify.finding.generated_config.sync",
		})
	} else {
		findings = append(findings, core.VerifyFinding{
			Code:       "verify.generated_config.sync",
			Severity:   core.SeverityFail,
			MessageKey: "verify.finding.generated_config.drift",
		})
	}

	if data, err := os.ReadFile(plan.PlanRecordPath); err == nil {
		findings = append(findings, core.VerifyFinding{
		...
		var savedPlan core.Plan
		if err := json.Unmarshal(data, &savedPlan); err == nil {
			findings = append(findings, core.VerifyFinding{
				Code:       "verify.plan_record.valid",
				Severity:   core.SeverityPass,
				MessageKey: "verify.finding.plan_record.valid",
			})
		} else {
			findings = append(findings, core.VerifyFinding{
				Code:       "verify.plan_record.valid",
				Severity:   core.SeverityWarn,
				MessageKey: "verify.finding.plan_record.invalid",
			})
		}
	} else {
		findings = append(findings, core.VerifyFinding{
...
func (s Service) Repair(applySafe bool, yes bool) (core.RepairResult, error) {
	...
	for index := range actions {
		...
		case "repair.profile.use_default":
			if applySafe {
				if _, err := s.UseProfile("default"); err != nil {
					return core.RepairResult{}, err
				}
				action.Applied = true
				appliedCount++
				_ = s.logLifecycle("repair", action.Code, "success", "", "default")
			}
		case "repair.generated_config.apply":
			fallthrough
		case "repair.generated_config.reconcile":
			if yes {
...
func (s Service) repairActions() []core.RepairAction {
	actions := make([]core.RepairAction, 0, 3)
	...
	if _, err := os.Stat(filepath.Join(s.profilesPath(), profileName+".yaml")); err != nil && profileName != "default" {
		actions = append(actions, core.RepairAction{
			Code:       "repair.profile.use_default",
			MessageKey: "repair.action.profile.use_default",
			Risky:      false,
		})
	}
	...
	} else if plan, err := s.Plan(); err == nil && len(plan.ContentDiffs) > 0 {
		actions = append(actions, core.RepairAction{
			Code:         "repair.generated_config.reconcile",
			MessageKey:   "repair.action.generated_config.reconcile",
			Risky:        true,
			RequiresFlag: "--yes",
		})
	}
...
func hasDoctorFailure(findings []core.DoctorFinding) bool {
...
func validateProfileTarget(profile core.Profile) error {
...
func (s Service) recordApplyState(result string, backupRecord core.BackupRecord) error {
	record, err := state.Load(state.DefaultStatePath(s.RootDir))
	if err != nil {
		return err
	}
	...
	now := time.Now().UTC()
	record.Version = "v1"
	if strings.TrimSpace(record.CurrentProfile) == "" {
		record.CurrentProfile = "default"
	}
...
func applyStateResult(findings []core.VerifyFinding) string {
	for _, finding := range findings {
		if finding.Severity == core.SeverityFail {
			return "fail"
		}
