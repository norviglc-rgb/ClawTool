...
func TestVerifyDetectsGeneratedConfigDriftAndInvalidSchema(t *testing.T) {
	t.Parallel()
	...
	configPath := filepath.Join(rootDir, ".clawtool", "cache", "effective-default.yaml")
	invalidConfig := "version: v1\nname: default\ntarget:\n  kind: ssh\n"
	...
	assertVerifyFinding(t, result.Findings, "verify.generated_config.schema", core.SeverityFail)
	assertVerifyFinding(t, result.Findings, "verify.generated_config.sync", core.SeverityFail)
}

func TestRepairSuggestsProfileFallbackAndReconcile(t *testing.T) {
	t.Parallel()
	...
	if err := os.Remove(filepath.Join(rootDir, ".clawtool", "profiles", "remote.yaml")); err != nil {
		t.Fatalf("remove profile: %v", err)
	}

	missingProfileRepair, err := service.Repair(false, false)
	if err != nil {
		t.Fatalf("repair missing profile: %v", err)
	}
	assertRepairAction(t, missingProfileRepair.Actions, "repair.profile.use_default")

	if _, err := service.UseProfile("default"); err != nil {
		t.Fatalf("use default: %v", err)
	}
	configPath := filepath.Join(rootDir, ".clawtool", "cache", "effective-default.yaml")
	driftedConfig := strings.ReplaceAll(string(mustReadFile(t, configPath)), "kind: local", "kind: ssh")
	if err := os.WriteFile(configPath, []byte(driftedConfig), 0o644); err != nil {
		t.Fatalf("write drifted config: %v", err)
	}

	reconcileRepair, err := service.Repair(false, false)
	if err != nil {
		t.Fatalf("repair reconcile: %v", err)
	}
	assertRepairAction(t, reconcileRepair.Actions, "repair.generated_config.reconcile")
}

func assertVerifyFinding(t *testing.T, findings []core.VerifyFinding, code string, severity core.Severity) {
	t.Helper()
	...
	t.Fatalf("missing verify finding %s in %+v", code, findings)
}

func assertRepairAction(t *testing.T, actions []core.RepairAction, code string) {
	t.Helper()
	...
	t.Fatalf("missing repair action %s in %+v", code, actions)
}
